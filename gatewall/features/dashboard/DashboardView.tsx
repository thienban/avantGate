...
export const DashboardView: React.FC = () => {
  ...
  const kpis = [
    ...
    },
    {
      ...
      icon: Layers,
      color: "from-indigo-500/20 to-violet-500/5 text-indigo-400 border-indigo-500/20",
    },
    {
      ...
      icon: Wrench,
      color: "from-blue-500/20 to-sky-500/5 text-sky-400 border-sky-500/20",
    },
    {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {kpis.map((kpi, idx) => {
          ...
          return (
            <Card
              key={idx}
              className="relative overflow-hidden border-zinc-800/80 bg-gradient-to-b from-zinc-900/60 to-zinc-950/60"
            >
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        ...
        <Card className="lg:col-span-2 border-zinc-800/80 bg-zinc-950/70 p-5">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h3 className="text-sm font-semibold text-white">Évolution des Dépenses & Runs</h3>
              <p className="text-xs text-zinc-400">Consommation journalière agrégée</p>
            </div>
        ...
        <Card className="border-zinc-800/80 bg-zinc-950/70 p-5">
          <h3 className="text-sm font-semibold text-white mb-1">Ventilation par Modèle LLM</h3>
          <p className="text-xs text-zinc-400 mb-4">Coût et jetons consommés</p>

          <div className="space-y-3.5">
            {Object.entries(finops.costByModel).map(([model, details]) => (
              <div
                key={model}
                className="p-3 rounded-lg border border-zinc-800/60 bg-zinc-900/40 text-xs"
              >
                <div className="flex items-center justify-between mb-1.5">
                  <span className="font-semibold text-zinc-200 truncate">{model}</span>
      ...
      <Card className="border-zinc-800/80 bg-zinc-950/70 p-5">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h3 className="text-sm font-semibold text-white">Consommation des Outils d&apos;Agents</h3>
            <p className="text-xs text-zinc-400">Surveillance des coûts et des anomalies</p>
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {toolHealth.slice(0, 3).map((tool) => (
            <div
              key={tool.toolName}
              className="p-3 rounded-lg border border-zinc-800/70 bg-zinc-900/40 flex flex-col justify-between"
            >
              <div className="flex items-center justify-between mb-2">
                <span className="font-semibold text-xs text-zinc-200">{tool.toolName}</span>
              ...
              <div className="space-y-1 text-[11px] text-zinc-400">
                <div className="flex justify-between">
                  <span>Succès :</span>
                  <span className="text-zinc-200 font-medium">{tool.successRate}%</span>
                </div>
                <div className="flex justify-between">
                  <span>Latence moy. :</span>
                  <span className="text-zinc-200 font-medium">{tool.avgDurationMs}ms</span>
                </div>
                <div className="flex justify-between">
