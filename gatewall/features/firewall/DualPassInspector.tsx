...
export const DualPassInspector: React.FC<DualPassInspectorProps> = ({ session }) => {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="p-4 rounded-xl border border-indigo-500/30 bg-gradient-to-r from-indigo-950/30 via-zinc-900/60 to-zinc-950/60 space-y-3">
        ...
        {clientAlerts.length > 0 ? (
          <div className="space-y-2">
            {clientAlerts.map((alert, idx) => (
              <div
                key={idx}
                className="p-3 rounded-lg border border-rose-500/40 bg-rose-950/30 flex items-start gap-2.5 text-xs text-rose-200"
              >
                ...
                <div className="flex-1 space-y-1">
                  ...
                  <p className="text-[11px] text-zinc-300">
                    ...
                    {alert.matchedPatternSnippet && (
                      <span className="font-mono ml-1 text-rose-300 bg-rose-950/60 px-1 py-0.5 rounded border border-rose-500/30">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 pt-1">
          {clientRenders.length > 0 ? (
            <div className="p-3 rounded-lg border border-cyan-500/30 bg-cyan-950/20 text-xs space-y-1">
              <div className="flex items-center justify-between">
            ...
            <div className="p-3 rounded-lg border border-zinc-800 bg-zinc-900/40 text-xs text-zinc-500">
              Aucun rendu de données directes sur ce run.
            </div>
          )}

          {session.userFeedback ? (
            <div className="p-3 rounded-lg border border-emerald-500/30 bg-emerald-950/20 text-xs space-y-1">
              <div className="flex items-center justify-between">
                ...
                {session.userFeedback.tag && (
                  <Badge variant="outline" className="text-[9px] text-emerald-300 border-emerald-500/30">
                    {session.userFeedback.tag}
                  </Badge>
                )}
              </div>
      ...
      <div className="space-y-3">
        ...
        {toolWithPii ? (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            ...
            <Card className="border-rose-500/30 bg-rose-950/10 p-4 space-y-2 rounded-xl">
              ...
              <pre className="p-3 rounded-lg bg-zinc-950/90 border border-rose-500/20 text-[11px] font-mono text-rose-200 overflow-x-auto max-h-44">
                {JSON.stringify(toolWithPii.rawPayload, null, 2)}
              </pre>
            </Card>

            {/* Channel B: Sanitized Summary for LLM (Green / Zero Leak) */}
            <Card className="border-emerald-500/30 bg-emerald-950/10 p-4 space-y-2 rounded-xl">
              ...
              <pre className="p-3 rounded-lg bg-zinc-950/90 border border-emerald-500/20 text-[11px] font-mono text-emerald-200 overflow-x-auto max-h-44">
                {JSON.stringify(toolWithPii.llmSummary, null, 2)}
              </pre>
            </Card>
          </div>
        ) : (
          <Card className="p-5 text-center text-xs text-zinc-500 rounded-xl border border-zinc-800 bg-zinc-900/30">
